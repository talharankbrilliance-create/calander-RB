...
const OFFICIAL_EVENTS = {
  6: {
    ...
    type: "National Day",
    holiday:
      "National day; not a gazetted public holiday since 2017",
    description:
...
export default function App() {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      (rows || []).forEach((row) => {
        const day = dateToDay(row.event_date);

        if (day) {
          map[day] = rowToEvent(row);
        }
      });
  ...
  // ─────────────────────────────────────────────
  const getEvent = (day) => {
    return OFFICIAL_EVENTS[day] || dbEvents[day] || null;
  };
      ...
      >
        ...
        >
          ...
          {!selected ? (
            ...
            >
              Select a date to view or add event
              details.
            </p>
          ) : editing === "add" ||
            editing === "edit" ? (
            ...
            <div>
              ...
              >
                ...
                <input
                  placeholder="Event type (e.g. Office Event)"
                  value={form.type}
                  onChange={(e) =>
                    setForm({
                      ...form,
